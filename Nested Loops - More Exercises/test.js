...
function solve(input) {
    ...
    let mashinPrice = Number(input[1]);
    let pupyPrice = Number(input[2]);

    let moneyBirthDay = 0;

    for (let curAge = 1; curAge <= liliAge; curAge++) {
        if (curAge % 2 === 0) {
            moneyBirthDay += ((curAge / 2) * 10 - 1);
        } else {
            moneyBirthDay += pupyPrice;
        }
    }

    if (moneyBirthDay >= mashinPrice) {
        console.log(`Yes! ${(moneyBirthDay - mashinPrice).toFixed(2)}`);
    } else {
        console.log(`No! ${(mashinPrice - moneyBirthDay).toFixed(2)}`);
    }
}
solve(["21",
"1570.98",
