...
function mine(input) {
    ...
    for (let curLocation = 1; curLocation <= countLocation; curLocation++) {
        let goldYield = 0;
        ...
        for (curDay = 1; curDay <= workDays; curDay++) {
            let dailyGoldYiel = Number(input[index]);
            goldYield += dailyGoldYiel;
            index++;

        }
        goldAverage = goldYield / workDays
        if (goldAverage >= expectedYield) {
            console.log(`Good job! Average gold per day: ${goldAverage.toFixed(2)}.`);
        } else {
            console.log(`You need ${(expectedYield - goldAverage).toFixed(2)} gold.`);
        }
    }
}
mine(["2",
